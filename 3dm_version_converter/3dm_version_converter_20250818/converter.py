...
RHINO_VERSIONS = {
    ...
    '5': 5,
    '6': 6,
    '7': 50,  # Rhino 7 uses file version 50
    '8': 60   # Rhino 8 uses file version 60
}
...
def convert_file(input_path, output_path, target_version):
...
def process_files(input_paths, output_dir, target_version, recursive=False):
    ...
    for input_path in tqdm(all_inputs, desc="Converting files"):
        ...
        # Convert the file
        success, error = convert_file(input_path, output_path, target_version)
        if success:
            processed += 1
        else:
...
def main(input_paths, output, version, recursive, overwrite):
    ...
    processed, errors = process_files(input_paths, output_path, target_version, recursive)
